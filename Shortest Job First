...
void sort_process_by_CPU_time(Process p[], int n);
...
void calculate_waiting_time(Process p[], int n)
{
    ...
    p[0].waiting_time = 0;
    for(i=1; i<n; i++) {
        p[i].waiting_time = p[i-1].waiting_time + p[i-1].CPU_time;
...
{
    int i, j;
    int last = p[n-1].CPU_time + ( n== 1 ? 0 : p[n-1].waiting_time);
    // printing top bar
    printf(" ");
    for(i=0; i<n; i++) {
        for(j=0; j<p[i].CPU_time; j++) printf("--");
        printf(" ");
    }
    printf("\n|");
    // middle position
    for(i=0; i<n; i++) {
        for(j=0; j<p[i].CPU_time-1; j++) printf(" ");
        printf("p%d", p[i].pid);
        for(j=0; j<p[i].CPU_time-1; j++) printf(" ");
        printf("|");
    }
    ...
    for(i=0; i<n; i++) {
        ...
        for(j=0; j<p[i].CPU_time-minus; j++) printf("  ");
